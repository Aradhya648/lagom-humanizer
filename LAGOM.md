...
- **Verified on live detectors:** Re-ran the 900-word essay through real QuillBot/ZeroGPT after the fix. **ZeroGPT stayed good at 8.8%. QuillBot barely moved: 82% (vs. 80% before the grammar fixes) — statistically unchanged.**
- **Confirms the hypothesis:** The flagged QuillBot text was grammatically clean this time ("It's making big changes in ways we couldn't have imagined...", "AI can give everyone a chance at good learning.") — no fragments, no broken semicolons, no lowercase starts. QuillBot is flagging the *style itself* — Gemini's flattened, plain-vocabulary paraphrase signature — not a bug in the deterministic passes. That lane is now exhausted for QuillBot specifically; the grammar fixes were still worth shipping (they're real correctness bugs affecting output quality and likely GPTZero/Originality too) but won't move QuillBot further on their own.
- **Decision:** Session stopped here by user choice. Next session should skip further deterministic-pass patching for QuillBot and go straight to a non-Gemini, non-GPT fingerprint-breaker (DeepSeek V3 or Kimi K2, both available via user's existing API keys) in the mid-chain slot vacated by the removed NVIDIA gpt-oss-20b pass — this is Day 6 of the roadmap, pulled forward because Day 1-5 deterministic work is now confirmed to have hit its ceiling for QuillBot.
- **Next action:** Wire DeepSeek V3 or Kimi K2 as `nvidiaFingerprintBreakPass`'s replacement (rename appropriately), test on the same two benchmark essays, compare against this session's numbers (QuillBot 82%/49%, ZeroGPT 8.8%/4%). If QuillBot drops meaningfully, build the Day 1 benchmark harness to lock in the win across more samples. If it doesn't move, the ceiling is likely inherent to the Gemini structural/semantic passes themselves — would justify moving to Tier 1 (N-best rescoring) or accepting QuillBot as a secondary target vs. GPTZero/Originality.
